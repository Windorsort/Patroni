...
# Add the PostgreSQL PRO repository
# Добавляем репозиторий PostgreSQL PRO
wget https://repo.postgrespro.ru/std/std-16/keys/pgpro-repo-add.sh
sh pgpro-repo-add.sh

# Install PostgreSQL PRO
# Установить PostgreSQL PRO
apt-get install postgrespro-std-16
systemctl status postgrespro-std-16.service
systemctl disable postgrespro-std-16.service
rm -rf /var/lib/pgpro/std-16/data/*
ls /var/lib/pgpro/std-16/data/
...
postgresql:
  ...
  data_dir: /var/lib/pgpro/std-16/data/
  bin_dir: /usr/bin/
  config_dir: /var/lib/pgpro/std-16/data/
...
listen master
    ...
    server PsqlPRO1 172.16.150.51:5432 check port 8008
    server PsqlPRO2 172.16.150.52:5432 check port 8008
    server PsqlPRO3 172.16.150.53:5432 check port 8008
